...
function TankMark:UpdateMobList()
    ...
    -- Note: ValidateDB is now shared or handled in logic
        ...
        if row then
            if index <= numItems then
                ...
                elseif data.type == "LOCK" then
                    TankMark:SetIconTexture(row.icon, data.mark); row.icon:Show()
...
function TankMark:CreateMobTab(parent)
    ...
    targetBtn:SetScript("OnClick", function()
        if UnitExists("target") then 
            ...
            if currentIcon then
                 ...
                 if TankMark.iconBtn and TankMark.iconBtn.tex then TankMark:SetIconTexture(TankMark.iconBtn.tex, currentIcon) end
            end
    ...
    iconTex:SetTexture("Interface\\TargetingFrame\\UI-RaidTargetingIcons"); TankMark:SetIconTexture(iconTex, TankMark.selectedIcon)
